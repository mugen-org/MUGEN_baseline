...
from models.gpt.attention import MultiHeadAttention
from models.gpt.utils import shift_dim
...
class VQVAE(pl.LightningModule):
    def __init__(self, args):
        ...
        kernel_size = args.kernel_size if hasattr(args, 'kernel_size') else 4
        self.encoder = Encoder(args.n_hiddens, args.n_res_layers, args.downsample, self.image_channels, kernel_size)
        self.decoder = Decoder(args.n_hiddens, args.n_res_layers, args.downsample, self.image_channels, kernel_size)
    ...
    def postprocess(self, video):
        video = torch.clamp(video, -0.5, 0.5) + 0.5
        video = (video * 255).permute(0, 2, 3, 4, 1).type(torch.uint8)
        return video
...
class Encoder(nn.Module):
    def __init__(self, n_hiddens, n_res_layers, downsample, image_channel=3, kernel_size=4):
        super().__init__()
        n_times_downsample = np.array([int(math.log2(d)) for d in downsample])
        self.convs = nn.ModuleList()
        max_ds = n_times_downsample.max()
        for i in range(max_ds):
            in_channels = image_channel if i == 0 else n_hiddens
            stride = tuple([2 if d > 0 else 1 for d in n_times_downsample])
            conv = SamePadConv3d(in_channels, n_hiddens, kernel_size, stride=stride)
...
class Decoder(nn.Module):
    def __init__(self, n_hiddens, n_res_layers, upsample, image_channel, kernel_size=4):
        super().__init__()
        self.res_stack = nn.Sequential(
            *[AttentionResidualBlock(n_hiddens)
              for _ in range(n_res_layers)],
        ...
        for i in range(max_us):
            out_channels = image_channel if i == max_us - 1 else n_hiddens
            us = tuple([2 if d > 0 else 1 for d in n_times_upsample])
            convt = SamePadConvTranspose3d(n_hiddens, out_channels, kernel_size,
                                           stride=us)
...
class SamePadConvTranspose3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, bias=True):
        ...
        self.convt = nn.ConvTranspose3d(in_channels, out_channels, kernel_size,
                                        ...
                                        output_padding=tuple([4 - k if s==2 else 0 for k, s in zip(kernel_size, stride)])) #only works for kernel=3 (besides default kernel=4) when stride=2
