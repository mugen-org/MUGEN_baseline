...
class ResNet(nn.Module):
    ...
    def preprocess(self, samples, samplerate):
        sr = 16000
        samples = F.interpolate(samples.mean(dim=-1, keepdim=True).transpose(1,2), int(16000*samples.shape[1]/samplerate))
        samples = samples + 0.5
        ...
        spectrograms = ((spectrograms - mean[:,None,None,None]) / (std[:,None,None,None] + 1e-9)).float()
        return spectrograms
